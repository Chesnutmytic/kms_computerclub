    ...
    if ($action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $conn->prepare(
            "INSERT INTO ARSIP_MATERI 
             (id_user, judul_dokumen, deskripsi, kategori, file_path, status, tgl_unggah) 
             VALUES (:user, :judul, :deskripsi, :kategori, :file, 'Pending', CURDATE())"
        )->execute([
        ...
        materiBack('success', 'Materi berhasil diunggah dan menunggu approval Super Admin.');
    }

    if (in_array($action, ['approve', 'reject'], true)) {
        if ($_SESSION['role'] !== 'Super Admin') {
            throw new RuntimeException('Hanya Super Admin yang dapat mengubah status materi.');
        }
        if ($id <= 0) {
